...
module.exports = function(grunt) {
    return {
        ...
        strippedframework: {
            expand: true,
            cwd: grunt.dirs.workspacesframework,
            src: ['**/*', '!lib/*', '!lib/fills/*', '!lib/angular/*', '!src/*', 'src/tmp/', 'src/DatasourceMap.js'],
        ...
        vistemplate: {
            ...
            dest: (grunt.dirs.workspacesvisualizations + '<%= projectName %>' + '/' + '<%= visualizationName %>' + '/' + '<%= visualizationName %>'),
            rename: function(dest, srcPath) {
        ...
        watchcopyproject: {
            ...
            dest: (grunt.dirs.deploy + '<%= projectName %>'),
            // options: {
            //     process: function(content, srcpath, trgtpath) {
            //         //TODO: Add this to a JSON file to allow reverse building
            //         console.log({
            //             'source': srcpath,
            //             'target': grunt.dirs.deploy.substring(0, grunt.dirs.deploy.length - 1) + srcpath.substring(grunt.dirs.workspacesprojects.length - 1, srcpath.length)
            //         })
            //     }
            // },
        ...
        watchcopyframework: {
            ...
            //         console.log({
            //             'source': srcpath,
            //             'target': grunt.dirs.deploy.substring(0, grunt.dirs.deploy.length - 1) + srcpath.substring(grunt.dirs.workspacesframework.length - 1, srcpath.length)
            //         })
            //     }
            // },            
        },
